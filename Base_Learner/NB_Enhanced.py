...
# Dataset
# ======================================================================================================================
base_nb_val_sentiment_df = pd.read_csv("Base_Learner/Results/NB/Base/base_nb_val_sentiment.csv")
...
# TEXT NORMALISATION
# -----------------------------------------------------------------------------
def normalise_for_spacy_nb(text):
    text = str(text)
...
def extract_regex_polarity_features_and_spans(text):
    ...
    normalised_text = normalise_for_spacy_nb(text)
...
def extract_polarity_feature_tokens(text):
    ...
    for caps_feature in caps_features:
        features.append("FEAT_" + caps_feature)
    ...
    for token in doc:
        ...
        if word in CONCESSIVE_STARTERS:
            ...
            features.append("FEAT_CONCESSIVE_" + clean_concession_marker)
...
def nb_append_polarity_features(text):
    original_text = normalise_for_spacy_nb(text)
    polarity_features = extract_polarity_feature_tokens(text)
    polarity_features = filter_nb_polarity_features(polarity_features)
    polarity_features = list(dict.fromkeys(polarity_features))
...
# -----------------------------------------------------------------------------
def get_registered_nb_rules_df():
    registered_rows = []
...
def classify_nb_effect(true_label, base_prediction, enhanced_prediction):
...
def create_nb_rule_review_audit_df(
        texts,
        true_labels,
        base_predictions,
    ...
    for review_index, text in enumerate(texts):
        ...
        changed_text = nb_append_polarity_features(text)

        polarity_features = extract_polarity_feature_tokens(text)
        filtered_polarity_features = filter_nb_polarity_features(polarity_features)
        filtered_polarity_features = list(dict.fromkeys(filtered_polarity_features))
        ...
        audit_rows.append({
            ...
            "true_class_probability_change": enhanced_true_probability - base_true_probability,
...
def get_nb_feature_operation_text(feature):
    rule_key = get_rule_key_from_feature(feature)
...
def print_short_nb_review_audit_summary(audit_df):
    ...
    print("\n========== SHORT NB REVIEW-LEVEL AUDIT SUMMARY ==========")
...
# FEATURE LEVEL AUDIT
# -----------------------------------------------------------------------------
def get_nb_feature_family(feature):
    if feature.startswith("FEAT_PHRASE_"):
        return "phrase"
...
def create_nb_feature_summary_df(nb_rule_review_audit_df):
    exploded_df = nb_rule_review_audit_df.explode("polarity_features")
    ...
    feature_summary_df["feature_family"] = feature_summary_df["feature"].apply(get_nb_feature_family)
...
def decide_nb_feature_action(row):
...
def add_nb_feature_decisions(feature_summary_df):
    feature_summary_df = feature_summary_df.copy()
    feature_summary_df["decision"] = feature_summary_df.apply(decide_nb_feature_action, axis=1)

    return feature_summary_df

def print_nb_feature_audit(feature_summary_df, top_n=100):
    print("\n========== NB FEATURE-LEVEL AUDIT ==========")
    ...
    family_summary_df["decisive_cases"] = (
        family_summary_df["corrected"]
        + family_summary_df["harmed"]
    )
...
# RULE LEVEL AUDIT
# -----------------------------------------------------------------------------
def create_nb_rule_summary_df(rule_review_audit_df):
    registered_rules_df = get_registered_nb_rules_df()
...
def print_nb_rule_audit(rule_summary_df):
...
def create_exclusive_nb_rule_summary_df(rule_review_audit_df):
    registered_rules_df = get_registered_nb_rules_df()
    overall_rule_summary_df = create_nb_rule_summary_df(rule_review_audit_df)
    ...
    exclusive_df["wrong_to_wrong_changed"] = ((exclusive_df["effect"] == "remained_wrong") & (exclusive_df["prediction_changed"]))
    ...
    exclusive_summary_df["decision"] = exclusive_summary_df.apply(decide_rule_action, axis=1)
...
def print_exclusive_nb_rule_summary(exclusive_summary_df):
...
# -----------------------------------------------------------------------------
def get_nb_rule_scope(rule_key):
    if rule_key is None:
        return None
...
def create_scoped_exclusive_nb_rule_summary_df(
    ...
    registered_rules_df = get_registered_nb_rules_df().copy()
    registered_rules_df["scope"] = registered_rules_df["rule_key"].apply(get_nb_rule_scope)

    scoped_registered_rules_df = (
        registered_rules_df[
    ...
    if scoped_registered_rules_df.empty:
        return pd.DataFrame()

    audit_df = rule_review_audit_df.copy()

    audit_df["target_scope_rules"] = audit_df["rule_keys"].apply(
        lambda rule_keys: [
            rule_key
            for rule_key in rule_keys
            if get_nb_rule_scope(rule_key) == target_scope
        ]
    )

    audit_df["number_of_target_scope_rules"] = audit_df["target_scope_rules"].apply(len)

    scoped_all_df = audit_df[
        audit_df["number_of_target_scope_rules"] > 0
    ].copy()

    if scoped_all_df.empty:
        scoped_summary_df = scoped_registered_rules_df.copy()

        scoped_summary_df["all_applications"] = 0
        scoped_summary_df["exclusive_applications"] = 0
        scoped_summary_df["corrected"] = 0
        scoped_summary_df["harmed"] = 0
        scoped_summary_df["prediction_changed"] = 0
        scoped_summary_df["decisive_changes"] = 0
        scoped_summary_df["net_corrections"] = 0
        scoped_summary_df["correction_precision"] = np.nan
        scoped_summary_df["mean_margin_change"] = np.nan
        scoped_summary_df["exclusive_status"] = "NO_SCOPED_EXCLUSIVE_EVIDENCE"
        scoped_summary_df["decision"] = "REVIEW"

        return scoped_summary_df

    scoped_all_exploded_df = scoped_all_df.explode("target_scope_rules")

    scoped_all_usage_df = (
        scoped_all_exploded_df
        .groupby("target_scope_rules", as_index=False)
        .agg(
            all_applications=("review_index", "nunique")
        )
        .rename(columns={
            "target_scope_rules": "rule_key"
        })
    )

    scoped_exclusive_df = audit_df[
        audit_df["number_of_target_scope_rules"] == 1
    ].copy()

    if scoped_exclusive_df.empty:
        scoped_summary_df = scoped_registered_rules_df.merge(
            scoped_all_usage_df,
            on="rule_key",
            how="left"
        )
    ...
    scoped_exclusive_df["exclusive_rule"] = scoped_exclusive_df["target_scope_rules"].apply(
        lambda rule_keys: rule_keys[0]
    )

    scoped_exclusive_df["wrong_to_wrong_changed"] = (
        (scoped_exclusive_df["effect"] == "remained_wrong")
        & (scoped_exclusive_df["prediction_changed"])
    )

    scoped_exclusive_usage_df = (
        scoped_exclusive_df
        .groupby("exclusive_rule", as_index=False)
        .agg(
            exclusive_applications=("review_index", "nunique"),
    ...
    scoped_summary_df["decisive_changes"] = (
        scoped_summary_df["corrected"]
        + scoped_summary_df["harmed"]
    )
    ...
    scoped_summary_df["exclusive_status"] = np.where(
        scoped_summary_df["exclusive_applications"] > 0,
        "HAS_SCOPED_EXCLUSIVE_EVIDENCE",
        "NO_SCOPED_EXCLUSIVE_EVIDENCE"
    )
...
def print_scoped_exclusive_nb_rule_summary(
        rule_review_audit_df,
        target_scope="phrase"
):
    scoped_summary_df = create_scoped_exclusive_nb_rule_summary_df(
    ...
    if scoped_summary_df.empty:
        print("No rules found for scope:", target_scope)
        return scoped_summary_df
...
def print_all_scoped_exclusive_nb_rule_summaries(rule_review_audit_df):
    scoped_tables = {}
    ...
    for target_scope in [
        ...
        scoped_tables[target_scope] = print_scoped_exclusive_nb_rule_summary(
            rule_review_audit_df=rule_review_audit_df,
            target_scope=target_scope
        )
...
# ----------------------------------------------------------------------------- START
# EXAMPLE REVIEW PRINTING
# -----------------------------------------------------------------------------
def nb_review_example_block(row):
    print("\n" + "-" * 120)
    ...
    if len(row["polarity_features"]) == 0:
        ...
        for feature in row["polarity_features"]:
            print("- " + get_nb_feature_operation_text(feature))
...
def nb_review_examples(title,selected_df,number=0):
    if number > 0:
    ...
    for _, row in selected_df.iterrows():
        nb_review_example_block(row)

def print_all_short_nb_review_examples(audit_df, number=0):
    corrected_df = audit_df[audit_df["effect"] == "corrected"]
    harmed_df = audit_df[audit_df["effect"] == "harmed"]
    wrong_to_wrong_df = audit_df[audit_df["effect"] == "wrong_to_wrong"]
    stayed_wrong_df = audit_df[audit_df["effect"] == "stayed_wrong"]
...
# ----------------------------------------------------------------------------- Start
# ENHANCED NB HYPERPARAMETER TUNING WITH OPTUNA
# ----------------------------------------------------------------------------- 
WORKER = min(8, os.cpu_count())
print("Precomputing Enhanced Validation Text...")
enhanced_text_val = pd.Series(
    process_map(
        nb_append_polarity_features,
        text_val.tolist(),
        max_workers=WORKER,
        chunksize=100,
        desc="Validation"
...
# ----------------------------------------------------------------------------- Start
# EVALUATE ENHANCED NB
# ----------------------------------------------------------------------------- 
print("\n========== ENHANCED NAIVE BAYES ==========")
enhanced_nb_study = optuna.create_study(
    direction='maximize',
    sampler=optuna.samplers.TPESampler(seed=42)
)
enhanced_nb_study.optimize(
    enhanced_nb_optuna,
    n_trials=20,
    n_jobs=1
)
...
if enhanced_nb_best['model_type'] == "multinomial":
    enhanced_nb_pipeline_steps.append(
        ('clf', MultinomialNB(
            alpha=enhanced_nb_best['alpha'],
            fit_prior=enhanced_nb_best['fit_prior']
        ))
    )
else:
    enhanced_nb_pipeline_steps.append(
...
enhanced_nb_uncalibrated_pipeline = ImbPipeline(enhanced_nb_pipeline_steps)
enhanced_nb_calibrated_pipeline = CalibratedClassifierCV(
    estimator=enhanced_nb_uncalibrated_pipeline,
    cv=calibration_cv,
    method="sigmoid",
    ensemble=False,
    n_jobs=3
)
enhanced_nb_pipeline = enhanced_nb_calibrated_pipeline

progress = tqdm(total=1, desc="Enhanced NB")
progress.set_description("Fitting Enhanced NB")
enhanced_nb_pipeline.fit(enhanced_text_train, sentiment_train)
progress.update(1)
progress.close()

enhanced_nb_val_sentiment = predict_with_progress(
    enhanced_nb_pipeline,
    enhanced_text_val,
    batch_size=5000,
    desc="Predicting Validation Sentiments With Enhanced NB"
)

enhanced_nb_test_sentiment = predict_with_progress(
    enhanced_nb_pipeline,
    enhanced_text_test,
    batch_size=5000,
    desc="Predicting Test Sentiments With Enhanced NB"
)
...
enhanced_nb_val_probabilities = predict_proba_with_progress(
    ...
    enhanced_text_val,
    batch_size=5000,
    desc="Predicting Validation Probabilities With Enhanced NB"
)

enhanced_nb_test_probabilities = predict_proba_with_progress(
    enhanced_nb_pipeline,
    enhanced_text_test,
    batch_size=5000,
    desc="Predicting Test Probabilities With Enhanced NB"
)

enhanced_nb_train_probabilities, enhanced_nb_probability_classes = cross_val_predict_proba_with_progress(
    enhanced_nb_pipeline,
    enhanced_text_train,
    sentiment_train,
    base_cv,
    desc="Predicting OOF Train Probabilities With Enhanced NB"
)
# ----------------------------------------------------------------------------- END

# ----------------------------------------------------------------------------- START
# NB AUDIT ON VALIDATION SET
# -----------------------------------------------------------------------------
nb_classes = np.array(["neg", "neu", "pos"])

assert np.array_equal(
    nb_classes,
    enhanced_nb_probability_classes
), "Class order mismatch!"

assert np.array_equal(
    nb_classes,
    enhanced_nb_pipeline.classes_
), "Class order mismatch!"

nb_rule_review_audit_df = create_nb_rule_review_audit_df(
    texts=text_val,
    true_labels=sentiment_val,
    base_predictions=base_nb_val_sentiment,
...
short_nb_review_summary_df = print_short_nb_review_audit_summary(nb_rule_review_audit_df)

nb_rule_summary_df = create_nb_rule_summary_df(nb_rule_review_audit_df)
print_nb_rule_audit(nb_rule_summary_df)

exclusive_nb_rule_summary_df = create_exclusive_nb_rule_summary_df(nb_rule_review_audit_df)
print_exclusive_nb_rule_summary(exclusive_nb_rule_summary_df)

nb_feature_summary_df = create_nb_feature_summary_df(nb_rule_review_audit_df)
nb_feature_summary_df = add_nb_feature_decisions(nb_feature_summary_df)
print_nb_feature_audit(nb_feature_summary_df, top_n=100)

scoped_exclusive_nb_rule_tables = print_all_scoped_exclusive_nb_rule_summaries(
    nb_rule_review_audit_df
)

print_all_short_nb_review_examples(nb_rule_review_audit_df, number=1)
# ----------------------------------------------------------------------------- END
...
enhanced_nb_train_probabilities_df = pd.DataFrame(enhanced_nb_train_probabilities).rename(columns={
    0: "enhanced_nb_neg",
    1: "enhanced_nb_neu",
    2: "enhanced_nb_pos"
})

enhanced_nb_val_probabilities_df = pd.DataFrame(enhanced_nb_val_probabilities).rename(columns={
    0: "enhanced_nb_neg",
    1: "enhanced_nb_neu",
    2: "enhanced_nb_pos"
})

enhanced_nb_test_probabilities_df = pd.DataFrame(enhanced_nb_test_probabilities).rename(columns={
    0: "enhanced_nb_neg",
    1: "enhanced_nb_neu",
    2: "enhanced_nb_pos"
})

enhanced_nb_train_probabilities_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_train_probabilities.csv"),
    index=False
)

enhanced_nb_val_probabilities_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_val_probabilities.csv"),
    index=False
)

enhanced_nb_test_probabilities_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_test_probabilities.csv"),
    index=False
)

print("\nSaved Naive Bayes Probabilities CSV Files to:", output_folder)

enhanced_nb_val_sentiment_df = pd.DataFrame({
    "enhanced_nb_sentiment": enhanced_nb_val_sentiment
})

enhanced_nb_test_sentiment_df = pd.DataFrame({
    "enhanced_nb_sentiment": enhanced_nb_test_sentiment
})

enhanced_nb_val_sentiment_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_val_sentiment.csv"),
    index=False
)

enhanced_nb_test_sentiment_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_test_sentiment.csv"),
    index=False
)

print("\nSaved Naive Bayes Sentiment CSV Files to:", output_folder)

output_folder = "Base_Learner/Rule_Decisions/NB"
os.makedirs(output_folder, exist_ok=True)

output = io.StringIO()
with contextlib.redirect_stdout(output):
    print_all_short_nb_review_examples(nb_rule_review_audit_df, number=0)
...
output = io.StringIO()
with contextlib.redirect_stdout(output):
    print_all_scoped_exclusive_nb_rule_summaries(nb_rule_review_audit_df)
text_output = output.getvalue()
...
output = io.StringIO()
with contextlib.redirect_stdout(output):
    print_short_nb_review_audit_summary(nb_rule_review_audit_df)
text_output = output.getvalue()
...
enhanced_nb_val_report_df = pd.DataFrame(
    classification_report(
        sentiment_val,
        enhanced_nb_val_sentiment,
        digits=4,
        output_dict=True
    )
).transpose().round(4)

enhanced_nb_test_report_df = pd.DataFrame(
    classification_report(
        sentiment_test,
...
enhanced_nb_val_report_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_validation_classification_report.csv"),
    index_label="class"
)

enhanced_nb_test_report_df.to_csv(
    os.path.join(output_folder, "enhanced_nb_test_classification_report.csv"),
    index_label="class"
)
...
table = ax.table(
    cellText=enhanced_nb_val_report_df.values,
    rowLabels=enhanced_nb_val_report_df.index,
    colLabels=enhanced_nb_val_report_df.columns,
    loc="center"
)
...
plt.savefig(
    os.path.join(output_folder, "enhanced_nb_validation_classification_report.png"),
    bbox_inches="tight",
    dpi=300
)
plt.close()
...
table = ax.table(
    cellText=enhanced_nb_test_report_df.values,
    rowLabels=enhanced_nb_test_report_df.index,
    colLabels=enhanced_nb_test_report_df.columns,
    loc="center"
)
...
plt.savefig(
    os.path.join(output_folder, "enhanced_nb_test_classification_report.png"),
    bbox_inches="tight",
    dpi=300
)
plt.close()
