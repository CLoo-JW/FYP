...
# ----------------------------------------------------------------------------- Start
# BASE META RANDOM FOREST
# -----------------------------------------------------------------------------
def base_meta_rf_optuna(trial):
...
print("\n========== BASE META RANDOM FOREST MODEL ==========")
base_meta_rf_study = optuna.create_study(
    direction='maximize',
    sampler=optuna.samplers.TPESampler(seed=42)
    )
base_meta_rf_study.optimize(
    base_meta_rf_optuna,
    n_trials=30
)

base_meta_rf_best = base_meta_rf_study.best_params

base_meta_rf_model = RandomForestClassifier(
    n_estimators=base_meta_rf_best['n_estimators'],
    max_depth=base_meta_rf_best['max_depth'],
    min_samples_split=base_meta_rf_best['min_samples_split'],
    min_samples_leaf=base_meta_rf_best['min_samples_leaf'],
    max_features=base_meta_rf_best['max_features'],
    random_state=42,
    n_jobs=-1
)
...
# ----------------------------------------------------------------------------- Start
# MIXED META RANDOM FOREST
# -----------------------------------------------------------------------------
def mixed_meta_rf_optuna(trial):
    optuna_rf_model = RandomForestClassifier(
        n_estimators=trial.suggest_int("n_estimators", 100, 800),
        max_depth=trial.suggest_int("max_depth", 3, 15),
        min_samples_split=trial.suggest_int("min_samples_split", 2, 20),
...
print("\n========== MIXED META RANDOM FOREST MODEL ==========")
mixed_meta_rf_study = optuna.create_study(
    direction='maximize',
    sampler=optuna.samplers.TPESampler(seed=42)
    )
mixed_meta_rf_study.optimize(
    mixed_meta_rf_optuna,
    n_trials=30
)

mixed_meta_rf_best = mixed_meta_rf_study.best_params

mixed_meta_rf_model = RandomForestClassifier(
    n_estimators=mixed_meta_rf_best['n_estimators'],
    max_depth=mixed_meta_rf_best['max_depth'],
    min_samples_split=mixed_meta_rf_best['min_samples_split'],
    min_samples_leaf=mixed_meta_rf_best['min_samples_leaf'],
    max_features=mixed_meta_rf_best['max_features'],
    random_state=42,
    n_jobs=-1
)
mixed_meta_rf_model.fit(mixed_meta_features_train, sentiment_train)
mixed_meta_rf_test_sentiment = mixed_meta_rf_model.predict(mixed_meta_features_test)
# ----------------------------------------------------------------------------- END

# ----------------------------------------------------------------------------- Start
# ENHANCED META RANDOM FOREST
# -----------------------------------------------------------------------------
def enhanced_meta_rf_optuna(trial):
    optuna_rf_model = RandomForestClassifier(
        n_estimators=trial.suggest_int("n_estimators", 100, 800),
        max_depth=trial.suggest_int("max_depth", 3, 15),
        min_samples_split=trial.suggest_int("min_samples_split", 2, 20),
...
print("\n========== ENHANCED META RANDOM FOREST MODEL ==========")
enhanced_meta_rf_study = optuna.create_study(
    direction='maximize',
    sampler=optuna.samplers.TPESampler(seed=42)
    )
enhanced_meta_rf_study.optimize(
    enhanced_meta_rf_optuna,
    n_trials=30
)

enhanced_meta_rf_best = enhanced_meta_rf_study.best_params

enhanced_meta_rf_model = RandomForestClassifier(
    n_estimators=enhanced_meta_rf_best['n_estimators'],
    max_depth=enhanced_meta_rf_best['max_depth'],
    min_samples_split=enhanced_meta_rf_best['min_samples_split'],
    min_samples_leaf=enhanced_meta_rf_best['min_samples_leaf'],
    max_features=enhanced_meta_rf_best['max_features'],
    random_state=42,
    n_jobs=-1
)
enhanced_meta_rf_model.fit(enhanced_meta_features_train, sentiment_train)
enhanced_meta_rf_test_sentiment = enhanced_meta_rf_model.predict(enhanced_meta_features_test)
# ----------------------------------------------------------------------------- END

# ----------------------------------------------------------------------------- Start
# READABLE RANDOM FOREST SHAP XAI FUNCTIONS
# -----------------------------------------------------------------------------
...
def save_readable_local_rf_shap_report(
    ...
    predicted_labels = model.predict(selected_feature_values)
    ...
    for local_position, row_index in enumerate(row_indices):
        ...
        predicted_sentiment = predicted_labels[local_position]
        ...
        explanation = (
            "The meta Random Forest predicted '"
...
# SAVE OUTPUT / XAI
# ----------------------------------------------------------------------------- 
xai_output_folder = "Meta_Model/XAI/RF"
os.makedirs(xai_output_folder, exist_ok=True)

base_readable_xai_report_df = save_readable_local_rf_shap_report(
...
print("\n========== BASE META RF CORRECT XAI EXAMPLES ==========")
...
print("\n========== BASE META RF WRONG XAI EXAMPLES ==========")
print(base_readable_xai_report_df[
...
print("\n========== MIXED META RF CORRECT XAI EXAMPLES ==========")
...
print("\n========== MIXED META RF WRONG XAI EXAMPLES ==========")
print(mixed_readable_xai_report_df[
...
enhanced_readable_xai_report_df = save_readable_local_rf_shap_report(
...
print("\n========== ENHANCED META RF CORRECT XAI EXAMPLES ==========")
...
print("\n========== ENHANCED META RF WRONG XAI EXAMPLES ==========")
print(enhanced_readable_xai_report_df[
...
print("\nSaved Random Forest SHAP XAI Report to:", xai_output_folder)

output_folder = "Meta_Model/Classification_Report/RF"
os.makedirs(output_folder, exist_ok=True)

base_meta_rf_test_report_df = pd.DataFrame(
...
enhanced_meta_rf_test_report_df = pd.DataFrame(
    classification_report(
        sentiment_test,
...
mixed_meta_rf_test_report_df.to_csv(
    os.path.join(output_folder, "mixed_meta_rf_test_classification_report.csv"),
    index_label="class"
)

enhanced_meta_rf_test_report_df.to_csv(
    os.path.join(output_folder, "enhanced_meta_rf_test_classification_report.csv"),
    index_label="class"
)
...
table = ax.table(
    cellText=base_meta_rf_test_report_df.values,
    rowLabels=base_meta_rf_test_report_df.index,
    colLabels=base_meta_rf_test_report_df.columns,
    loc="center"
)
...
plt.savefig(
    os.path.join(output_folder, "base_meta_rf_test_classification_report.png"),
    bbox_inches="tight",
    dpi=300
)
plt.close()
...
table = ax.table(
    cellText=mixed_meta_rf_test_report_df.values,
    rowLabels=mixed_meta_rf_test_report_df.index,
    colLabels=mixed_meta_rf_test_report_df.columns,
    loc="center"
)
...
plt.savefig(
    os.path.join(output_folder, "mixed_meta_rf_test_classification_report.png"),
    bbox_inches="tight",
    dpi=300
)
plt.close()
...
table = ax.table(
    cellText=enhanced_meta_rf_test_report_df.values,
    rowLabels=enhanced_meta_rf_test_report_df.index,
    colLabels=enhanced_meta_rf_test_report_df.columns,
    loc="center"
)
...
plt.savefig(
    os.path.join(output_folder, "enhanced_meta_rf_test_classification_report.png"),
    bbox_inches="tight",
    dpi=300
)
plt.close()
